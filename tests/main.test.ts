...
describe('Main Controller 마이그레이션 테스트', () => {
    ...
    it('메인 페이지 호출 시 최신 상품 8개를 조회하고 정상적으로 렌더링해야 한다', async () => {
        const mockProducts = [
            { id: 1, name: '다이아몬드 목걸이', price: 1000000, created_at: new Date(), images: [] },
            { id: 2, name: '사파이어 반지', price: 500000, created_at: new Date(), images: [] }
        ];
        ...
        expect(mockRepo.find).toHaveBeenCalledWith(expect.objectContaining({ take: 8 }));
    });
